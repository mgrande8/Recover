...
import { Share2, Moon, Clock, Target } from 'lucide-react';
import { Button } from '@/components/ui';
import * as htmlToImage from 'html-to-image';
import { formatDuration } from '@/lib/utils';
import type { MonthlyReportStats } from '@/types';
...
export function ShareReport({ stats, periodStart, periodEnd }: ShareReportProps) {
  const cardRef = useRef<HTMLDivElement>(null);
  const [isSharing, setIsSharing] = useState(false);

  const startFormatted = new Date(periodStart).toLocaleDateString('en-US', {
    month: 'short',
    day: 'numeric',
  });
  const endFormatted = new Date(periodEnd).toLocaleDateString('en-US', {
    month: 'short',
    day: 'numeric',
  });
      ...
      <div className="fixed -left-[9999px] top-0">
        <div ref={cardRef} className="w-[400px] bg-gradient-to-br from-[#0a0a0f] via-[#0f0f2a] to-[#0a0a0f] p-6">
          <div className="text-center mb-6">
            ...
            <p className="text-xs text-text-muted">{startFormatted} — {endFormatted}</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            ...
            <div className="bg-background/50 rounded-xl p-3 text-center">
              ...
              <p className="text-lg font-bold text-text-primary">{stats.consistencyScore}</p>
              <p className="text-xs text-text-muted">Consistency</p>
            </div>
            <div className="bg-background/50 rounded-xl p-3 text-center">
              <p className="text-lg font-bold text-text-primary">{stats.nightsLogged}</p>
